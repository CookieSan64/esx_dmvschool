...
function OpenDMVSchoolMenu()
	...
	if not ownedLicenses['dmv'] then
		elements[#elements+1] = {
			icon = "fas fa-car",
			title = (('%s: <span style="color:pink;">%s</span>'):format(TranslateCap('theory_test'), TranslateCap('school_item', ESX.Math.GroupDigits(Config.Prices['dmv'])))),
			value = "theory_test"
		}
	end
	...
	if ownedLicenses['dmv'] then
		if not ownedLicenses['drive'] then
			elements[#elements+1] = {
				icon = "fas fa-car",
				title = (('%s: <span style="color:pink;">%s</span>'):format(TranslateCap('road_test_car'), TranslateCap('school_item', ESX.Math.GroupDigits(Config.Prices['drive'])))),
				value = "drive_test",
				type = "drive"
			}
		end
		...
		if not ownedLicenses['drive_bike'] then
			elements[#elements+1] = {
				icon = "fas fa-car",
				title = (('%s: <span style="color:pink;">%s</span>'):format(TranslateCap('road_test_bike'), TranslateCap('school_item', ESX.Math.GroupDigits(Config.Prices['drive_bike'])))),
				value = "drive_test",
				type = "drive_bike"
			}
		end
		...
		if not ownedLicenses['drive_truck'] then
			elements[#elements+1] = {
				icon = "fas fa-car",
				title = (('%s: <span style="color:pink;">%s</span>'):format(TranslateCap('road_test_truck'), TranslateCap('school_item', ESX.Math.GroupDigits(Config.Prices['drive_truck'])))),
				value = "drive_test",
				type = "drive_truck"
			}
		end
        ...
        if CurrentTest == 'drive' then
            ...
            if IsPedInAnyVehicle(playerPed, false) then
                ...
                for k,v in pairs(Config.SpeedLimits) do
                    if CurrentZoneType == k and speed > v then
                        ...
                        if not IsAboveSpeedLimit then
                            ...
                            -- Message d'erreur personnalisé pour la vitesse excessive
                            ESX.ShowNotification("Hé Sébastien Loeb, on n'est pas sur un circuit !")
                            ESX.ShowNotification(TranslateCap('errors', DriveErrors, Config.MaxErrors))
                        end
                    end
                end
                ...
                if health < LastVehicleHealth then
                    ...
                    -- Message d'erreur personnalisé pour les dommages au véhicule
                    ESX.ShowNotification("Hé, tu te crois à la fête foraine ? C'est pas les auto-tamponneuses ici !")
                    ESX.ShowNotification(TranslateCap('errors', DriveErrors, Config.MaxErrors))

                    -- éviter l'empilement des erreurs
                    LastVehicleHealth = health
                    sleep = 1500
                end
            end
